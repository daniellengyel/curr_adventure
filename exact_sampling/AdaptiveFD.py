...
def _adapt_FD(f, x_0, x_dir, sig, rl, ru, jrandom_key, f_x_0=None, return_h=False):
    ...
    if return_h:
        return h, total_func_calls
    else:
        return (f_x_0_h - f_x_0)/h, total_func_calls

def create_sampling_set(f, x_0, sig, jrandom_key, rl=1.5, ru=6):
    dim = len(x_0)
    ...
    for i in range(dim):
        curr_dir = all_dirs[i]
        jrandom_key, subkey = jrandom.split(jrandom_key)
        S[i, i], _ = _adapt_FD(f, x_0, curr_dir, sig, rl, ru, subkey, f_x_0, return_h=True)
